...
CREATE TABLE IF NOT EXISTS automation.ingestion_log (
    ...
    -- Logical source name.
    -- Examples: fish, insects, fossils, villagers, housewares, recipes.
    source_name TEXT NOT NULL,

    -- Relative path of the CSV file inside the project.
    -- Example: data/raw/fish/fish01.csv
...
--
-- Why only SUCCESS?
-- A file can fail multiple times, but it can be marked as successfully
-- loaded only once. This is what lets ingest_files.py safely skip a file
-- it has already processed, while still allowing a retry after a failure.
